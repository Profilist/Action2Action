...
class GestureRecognizer:
    def __init__(
        ...
        self._motion_history: Dict[str, Deque[Tuple[float, float, int]]] = {
            "Left": deque(),
            "Right": deque(),
        }
        self._swipe_window_size: int = 10
        self._swipe_speed_thresh: float = (
            0.6  # normalized units per second (easier to trigger)
        )
        ...
        # Debug metrics
        self._debug_swipe_metrics: Dict[str, Dict[str, float]] = {
            "Left": {},
            "Right": {},
        }
                    ...
                    if buf and len(buf) >= 2:
                        ...
                        if m:
                            ...
                            cv2.putText(
                                frame,
                                text,
                                (12, 84 if hand_id == "Left" else 108),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.5,
                                (200, 255, 0),
                                2,
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> List[GestureEvent]:
        ...
        for hand_landmarks, handedness in zip(
            ...
            static_events = self._classify_static(
    ...
    def _update_motion_history(self, handedness: str, lm) -> None:
        ...
        buf = self._motion_history.setdefault(
            handedness, deque(maxlen=self._swipe_window_size)
        )
        # Ensure maxlen is set
        if buf.maxlen != self._swipe_window_size:
            self._motion_history[handedness] = deque(
                buf, maxlen=self._swipe_window_size
            )
            buf = self._motion_history[handedness]
        buf.append((x, y, t))

    def _detect_dynamic(
        self, handedness: str, lm, palm_scale: float
    ) -> List[GestureEvent]:
        events: List[GestureEvent] = []
        # Swipes
    ...
    def _detect_swipe(self, handedness: str) -> Optional[str]:
        ...
        self._debug_swipe_metrics[handedness] = {
            "disp": float(disp),
            "speed": float(speed),
        }
    ...
    def _detect_pinch_hold_release(
        self, handedness: str, lm, palm_scale: float
    ) -> List[GestureEvent]:
        events: List[GestureEvent] = []
        now_ms = int(time.time() * 1000)
    ...
    def _finger_states(self, lm, palm_scale: float) -> Dict[str, bool]:
        ...
        def extended(tip_idx: int, pip_idx: int) -> bool:
            return (lm[tip_idx].y < lm[pip_idx].y) and (
                np.linalg.norm(np.array([lm[tip_idx].x, lm[tip_idx].y]) - wrist)
                > 0.6 * palm_scale
            )
        ...
        states = {
            "thumb": lm[4].x < lm[3].x
            if True
            else False,  # placeholder lateral check in mirrored space
            "index": extended(8, 6),
    ...
    def _classify_static(
        ...
        # point
        if finger_states["index"] and not (
            finger_states["middle"] or finger_states["ring"] or finger_states["pinky"]
        ):
            conf = self._score(hand_score, margin=0.5)
        ...
        if pinch_on or (pinch_state and not pinch_off):
            margin = max(
                0.0,
                (self._pinch_exit * palm_scale - pinch_dist)
                / (self._pinch_exit * palm_scale),
            )
    ...
    def _smooth(self, event: GestureEvent) -> GestureEvent:
        ...
        return GestureEvent(
            event.type, event.handedness, smoothed, event.timestamp_ms, event.features
        )
    ...
    def load_config(self, path: str) -> None:
        ...
        # Dynamic params
        self._swipe_window_size = int(
            cfg.get("swipe_window_size", self._swipe_window_size)
        )
        self._swipe_speed_thresh = float(
            cfg.get("swipe_speed_thresh", self._swipe_speed_thresh)
        )
        self._swipe_min_cosine = float(
            cfg.get("swipe_min_cosine", self._swipe_min_cosine)
        )
        self._swipe_min_displacement = float(
            cfg.get("swipe_min_displacement", self._swipe_min_displacement)
        )
        ...
        if any(v is not None for v in (mdc, mtc, mnh)):
            print(
                "Note: min_* confidences and max_num_hands require re-instantiation to take effect."
            )
